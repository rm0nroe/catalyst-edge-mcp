...
- The working tree is intentionally uncommitted and contains the recorded execution pack plus preserved unrelated `thoughts/` artifacts. This prevents a clean-commit release claim and keeps R1/R2 partial.
...
| --- | --- | --- |
| R0 scope freeze | **Met locally** | PRD/TDD/GTM retain two read-only local tools, one ticker per score call, deterministic unbacktested scoring, typed missingness, and no recommendation. |
