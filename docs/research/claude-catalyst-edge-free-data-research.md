...
**Date:** July 13, 2026 | **Against:** PRD.md + TDD.md (deterministic_v1 scorer, 5 canonical families)

## 0. Framing against the actual TDD

The TDD already locks in a specific dependency shape:

| Family | Current TDD source | Paid? | Failure mode if unfunded |
|---|---|---|---|
...
| 3 | **Finnhub `/company-news`** — genuinely free tier, distinct from the paid `/stock/social-sentiment` endpoint your TDD already keys to `FINNHUB_API_KEY` | Headline, summary, URL, source, datetime | REST, `X-Finnhub-Token` header, same key you already provision for social | Free tier: 60 calls/min, no separate cost | 60 req/min free; near-real-time | Well-documented, enterprise-run (ex-Google/Bloomberg team), widely used | **Low** (you already have the adapter and the key — this is one more endpoint call) | Second confirming news source, reduces reliance on FMP news specifically | Still an aggregator, not primary; treat at ~0.80 quality like your existing Finnhub row |
| — | Company IR-page / press-release RSS (per-ticker) | Primary press releases | No universal free API — would require a hand-maintained ticker→RSS-URL map | N/A | N/A | Fragile at scale | **High** for broad coverage | Marginal | Not recommended for v1 — same "don't scope-creep like MediaCrawler" logic your PRD already applies elsewhere. Revisit only if a specific high-value ticker needs it. |

**Verdict — Need 2: largely achievable for free.** SEC 8-K (already free) + GDELT (new, zero-cost, legally clean because it never stores full text) gives you a second independent, dedup-capable `filings_news` source that satisfies the TDD §14 launch-readiness bar ("at least one fresh directional family from FMP, Finnhub, or a true-flow provider") without spending on FMP news specifically. Finnhub's already-free news endpoint is a good third leg using a key you already have.
...
|---|---|---|---|---|
| 1 | yfinance chain snapshot (**already in your TDD**, `degraded=true`, confidence capped 0.45) | Current call/put open interest and volume per contract, no trade-level ticks, no sweep/block, no aggressor side | Yes | Correctly the best free option; nothing below beats it |
...
**Verdict — Need 3: not achievable for free at the bar you defined** (sweeps, blocks, premium, aggressor side, volume vs. OI at the trade level). This is exactly why your TDD already treats yfinance as degraded rather than as flow — that design is correct and there is no free way to close the gap. The only way to get real flow is to pay FlowAlgo, CheddarFlow, a Polygon paid tier, Databento, or another OPRA-licensed reseller — your TDD already accounts for this correctly via `CATALYST_EDGE_OPTIONS_PROVIDER`.
...
Changes relative to TDD §2/§6, keeping the existing adapter protocol and family budgets untouched:
...
2. **`gdelt.py` + extend `finnhub.py` with the free `/company-news` call.** Dual-sources the `filings_news` family, wires into your existing `(family, signal, timestamp, canonical_source_url)` dedup key, satisfies TDD §14 launch-readiness without paying for FMP news.
