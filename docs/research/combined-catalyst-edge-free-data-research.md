...
**Status:** reconciled recommendation for PRD/TDD revision and implementation
...
| Current launch-readiness test | The TDD currently hard-codes SEC plus a fresh directional FMP, Finnhub, or true-flow provider. New free sources do not satisfy that gate until the TDD and smoke tests are intentionally revised. |
...
- Preserve the TDD definition that two distinct insiders in one direction form a cluster, but distinguish cluster strength:
...
## Required TDD revisions before implementation

The current TDD assumes FMP/Finnhub keys and a provider-name-based launch gate. The new architecture requires an explicit design revision before code changes.
...
### Phase 0 — revise the TDD and contracts
