...
4. Draft the provider-neutral TDD addendum now for immutable manifests, historical-availability proof, registry/policy/scorer hashes, replay cutoff logic, security lifecycle, labels, and audit fixtures. Defer vendor-specific mappings until rights/fields are confirmed.
...
**Go** on preparing the vendor-rights package and provider-neutral Stage A TDD. **Conditional go** on Stage A after identity/lifecycle/price access is approved. **No-go** on Stage B, calibration, or any predictive claim until a survivor-aware terminal-outcome source is contracted and the Stage A timestamp audit passes.
