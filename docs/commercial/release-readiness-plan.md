...
| R5 — release sample | Five fixed public tickers produce five schema-valid dossiers, complete available claim pagination, and explicit missing/rejected evidence; one sanitized example is safe to publish | Fresh AAPL/NVDA/TSLA/RKLB/BRK.B SEC-only calls are schema-valid and contain typed missingness. No grouped claim was available, so the explicit no-claim boundary remains. The sanitized AAPL source copy now has three website-hero directions, two social crops, and one explanatory infographic under `docs/validation/launch-assets/catalyst-edge-local-beta/`; Ryan's exact visual and copy approval remains open. |
