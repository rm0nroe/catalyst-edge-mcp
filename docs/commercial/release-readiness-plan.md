...
| --- | --- | --- |
| R0 — scope freeze | product specification/technical specification/GTM agree on two local read-only tools, one ticker per call, deterministic unbacktested scoring, typed missingness, and no recommendation | Met after publication documentation and configuration recheck. |
...
| R3 — public configuration | SEC identity required for SEC; attributed GDELT discovery enabled; issuer feeds and Bluesky disabled; package metadata, `.env.example`, README, and runtime agree | Implemented for v0.1.4. Dossiers, claim pages, refresh output, and health output include the mandatory GDELT citation/link; the public configuration retains an explicit GDELT opt-out. |
...
- attributed GDELT discovery enabled by default, with issuer feeds, Bluesky, options, and sentiment disabled;
